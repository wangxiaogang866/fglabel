function [node_xyz, face_node, group_idxes]= obj__read( input_file_name )
...
%  Get sizes.
%
[ node_num, face_num, normal_num, order_max,group_idxes] = obj_size ( input_file_name );
%
%  Get the data.
%
...
function [ node_num, face_num, normal_num, order_max, group_idxes] = obj_size ( input_file_name )
...
while ( 1 )
    ...
    if ( s_eqi ( word_one, 'o' ) ) || ( s_eqi ( word_one, 'g' ) )
        if begin_group==0&&g_begin<g_end
            group_idxes = [group_idxes; g_begin, g_end-1];
            g_begin = g_end;
        else
            begin_group =0;
        end
