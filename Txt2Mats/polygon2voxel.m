function Volume=polygon2voxel(FV,VolumeSize,scaling,positive,offset,mode,Yxz,hollow)
...
%VolumeSize=[max(VolumeSize) max(VolumeSize) max(VolumeSize)];
...
switch(lower(mode(1:2)))
    ...
    case{'cu'} % custom
        % Make all vertices-coordinates positive
        FV.vertices=bsxfun(@minus, FV.vertices, positive);
        scaling=scaling;
        % Make the vertices-coordinates to range from 0 to 100
        FV.vertices=FV.vertices*scaling+1;
        offset =offset ;
